# New mobility method that only takes into account the
...
def mid_game_heuristic(board, player, opponent):
    ...
    board_score = dynamic_weights_score(board, player, opponent)
    mobility_score = mobility(board, player, opponent)
    return mobility_score*20 + corners_score*20 + board_score + near_corner_penalty*100 #*10

def end_game_heuristic(board, player, opponent):
    (corners_score, near_corner_penalty) = corner_capture(board, player, opponent)
    parity_score = parity(board, player, opponent)
    #board_score = dynamic_weights_score(board, player, opponent)
    return parity_score*20 + corners_score*10 + near_corner_penalty*100
...
@register_agent("near_corner")
class NearCorner(Agent):

    def __init__(self):
        super(NearCorner, self).__init__()
        self.name = "NearCorner"
