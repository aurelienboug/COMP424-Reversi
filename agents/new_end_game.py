...
def corner_capture(board, player, opponent):
    ...
    return ratio(player_corner_capture,opponent_corner_capture), -near_corner_player
...
def start_game_heuristic(board, player,opponent):
    ...
    return mobility_score*20 + board_score + parity_score + corners_score*100 + near_corner_penalty*100

def mid_game_heuristic(board, player, opponent):
    (corners_score, near_corner_penalty) = corner_capture(board, player, opponent)
    board_score = dynamic_weights_score(board, player, opponent)
    mobility_score = mobility(board, player, opponent)
    return mobility_score*20 + corners_score*20 + board_score + near_corner_penalty*100

def end_game_heuristic(board, player, opponent):
    parity_score = parity(board, player, opponent)
    stability_board = calculate_stability(board, player, opponent)
    stability_score = ratio(np.sum((board == player) * stability_board) , -np.sum((board == opponent) * stability_board))
    return parity_score*20 + stability_score
...
def heuristic(board, player, opponent):
    ...
    elif not all_corners_capture(board):
...
@register_agent("new_end_game")
class NewEndGame(Agent):

    def __init__(self):
        super(NewEndGame, self).__init__()
        self.name = "NewEndGame"
