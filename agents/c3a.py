...
def board_stability(board, player, opponent):
    ...
    stability_map = np.full_like(board, 0, dtype=object)
...
def corner_capture(board, player, opponent):
    ...
    if (player_corner_capture + opponent_corner_capture) != 0:
        score = (player_corner_capture - opponent_corner_capture) / (
                    player_corner_capture + opponent_corner_capture) * 100
    return score
...
def heuristic(board, player, opponent):
    ...
    current_weights = board_weights(board.shape[0])*board_stability(board,player,opponent)
    placement_score = np.sum((board == player) * current_weights) - np.sum((board == opponent) * current_weights)
    return parity_score * 25 + corners_score * 30 + placement_score * 25
...
class C3a(Agent):
    ...
    def step(self, board, player, opponent):
        ...
        value, best_move = alpha_beta(board, 3, float('-inf'), float('inf'), True, player, opponent)
        ...
        # print(board_stability(board, player, opponent))
        # print(board_weights(board.shape[0])*board_stability(board,player,opponent))
        return best_move
...
def mobility(board, player, opponent):
    ...
    if player_player_mobility + opponent_player_mobility != 0:
        score = (player_player_mobility - opponent_player_mobility) / (player_player_mobility + opponent_player_mobility) *100
    return score
