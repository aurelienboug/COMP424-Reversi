...
def corner_capture(board, player, opponent):
    ...
    return -opponent_corner_capture, -near_corner_player
...
def start_game_heuristic(board, player, opponent):
    ...
    return mobility_score + parity_score + corners_score*50 + near_corner_penalty*50 #+ board_score

def mid_game_heuristic(board, player, opponent):
    (corners_score, near_corner_penalty) = corner_capture(board, player, opponent)
    #board_score = dynamic_weights_score(board, player, opponent)
    mobility_score = mobility(board, player, opponent)
    return mobility_score + corners_score*50 + near_corner_penalty*50  #+ board_score
...
def end_game_heuristic(board, player, opponent):
    ...
    parity_score = parity(board, player, opponent)
    # board_score = dynamic_weights_score(board, player, opponent)
    return parity_score + corners_score*50  + near_corner_penalty*50 #+board_score
...
@register_agent("only_loses")
class OnlyLoses(Agent):

    def __init__(self):
        super(OnlyLoses, self).__init__()
        self.name = "OnlyLoses"
