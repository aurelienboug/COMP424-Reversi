...
def start_game_heuristic(board, player,opponent):
    ...
    return mobility_score*10 + board_score + parity_score
...
def end_game_heuristic(board, player, opponent):
    ...
    weights = board_weights(board.shape[0])
    board_score = (np.sum((board == player) * weights) - np.sum((board == opponent) * weights))
    return parity_score*10 + board_score + stability_score*10 + corners_score*10 + near_corner_penalty"""
...
def dynamic_weights_board(board, player, opponent):
    #return calculate_stability(board, player, opponent)
    return board_weights(board.shape[0])*calculate_stability(board,player,opponent)


def heuristic(board, player, opponent):
    board_score = dynamic_weights_board(board, player, opponent)
    score = (np.sum((board == player) * board_score) + np.sum((board == opponent) * board_score))
    return score
...
@register_agent("dynamic_weighting")
class Dynamic_wieghting(Agent):

    def __init__(self):
        super(Dynamic_wieghting, self).__init__()
        self.name = "Dynamic_wieghting"
    ...
    def step(self, board, player, opponent):
        ...
        print(calculate_stability(board, player, opponent))
        print(board_weights(board.shape[0]))
        print(board_weights(board.shape[0])*calculate_stability(board, player, opponent))"""
